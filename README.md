...
This project is a comprehensive, data-driven investigation into *why* the collapse happened. By extracting and analyzing telemetry for all 82 games via the NHL's live API, this analysis debunks popular fan narratives (e.g., "the defense quit") and proves that the collapse was a perfect storm of **physical fatigue, psychological pressure, strength of schedule, and goaltending regression.**
...
### 2. The Special Teams "Freeze" (Psychological Pressure)
Power Play goals cratered from 28 to just 13 down the stretch. To measure *why*, I built a custom **Pass-to-Shoot Ratio** proxy metric.
* **The Data:** The Pass-to-Shoot ratio for the core PP unit spiked from **0.200 to 0.222**, and unforced giveaways per 60 minutes increased from **2.87 to 2.98**.
* **The Montreal Benchmark:** By comparing this to historical data from the Montreal Canadiens during peak playoff scrutiny, we see an identical "turtle shell" psychological response. Under immense pressure, players abandon aggressive shoot-first mentalities and default to over-passing and perimeter hesitation.
...
Offensive struggles only tell half the story. A holistic view of the team's system reveals the true fatal blow.
...
1. **Navigating "Phantom Keys" & Deprecations:** The 2026 API update quietly deprecated the `lastName` key in the boxscore endpoint. I engineered a custom parsing layer to extract the localized full-name string (`{"name": {"default": "Dylan Larkin"}}`) and split it dynamically for cohort mapping.
2. **Building Proxy Metrics:** When the API ceased serving `powerPlayToi` for individual skaters, I pivoted from measuring *Shots per PP Minute* to building the **Pass-to-Shoot Ratio** using raw Assists/SOG keys. This proxy metric ultimately provided a stronger behavioral insight into player hesitation.
3. **Handling Broken Goalie Strings (Anti-Crash Logic):** When goalies are pulled mid-game, the API collapses standard integers into fraction strings (e.g., `"8/13"`). I built an anti-crash string-splitter to detect fractions, slice the string, and cast the integers to maintain the integrity of the 90-game extraction loop.
4. **Dynamic Quality of Competition (QoC):** To accurately measure the Strength of Schedule without observer bias, I avoided hardcoded lists. The script queries the league standings endpoint for the *day prior to puck drop*. If the opponent had a Points Percentage $\ge$ .600 on that specific date (minimum 10 GP), the game was dynamically flagged as a Contender matchup.
